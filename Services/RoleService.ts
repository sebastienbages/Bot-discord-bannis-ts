...
export class RoleService {
	...
	public async assignServerRole(selectMenuInteraction: SelectMenuInteraction): Promise<void> {
		await selectMenuInteraction.deferReply({ ephemeral: true });
		...
		await selectMenuInteraction.followUp({ content: "Ok c'est parti ! Accroche ta ceinture ça va secouer :rocket:", files: [ teleportationImage ], ephemeral: true, fetchReply: false });
		await wait(8000);
		await selectMenuInteraction.editReply({ content: `Te voilà arrivé sur le discord complet :partying_face: \nTu appartiens au serveur ${serverNumber} :sunglasses: \nD'ailleurs, je me suis permis de l'écrire à côté de ton pseudo :relaxed:`, attachments: [] });
