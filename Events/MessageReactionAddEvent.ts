...
export class MessageReactionAddEvent {
	...
	public async run(messageReaction: MessageReaction, user: User): Promise<void> {
		...
		// TICKET
		if ((emoji != TicketService.createReaction || emoji != TicketService.closeReaction || emoji != TicketService.reOpenTicketReaction || emoji != TicketService.deleteTicketReaction) && targetChannel.id === ticketConfig.ChannelId) {
